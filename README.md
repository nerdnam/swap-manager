<p align="center">
<img src="https://raw.githubusercontent.com/nerdnam/swap-manager/main/readme/favicon.png"  width="128" height="128">
</p>
...
<p align="center">
<img src="https://raw.githubusercontent.com/nerdnam/swap-manager/main/readme/webui.png" >
</p>
...
1.  **환경 변수 설정 (`swap.env` 파일 생성):**
    ...
    MEMORY_LIMIT=8G # 예시 값 변경
    # Cgroup을 통해 설정할 스왑 제한 (SWAP_SIZE와 동일하게 설정 가능)
    SWAP_LIMIT=64G # SWAP_SIZE 환경 변수를 참조하는 대신 명시적 값 권장
    ...
    RESOURCE_CHECK_INTERVAL=30 # 예시 값 변경
    ...
    ```
    **참고:** `SWAP_LIMIT=${SWAP_SIZE}`와 같이 환경 변수 내에서 다른 환경 변수를 참조하는 기능은 쉘 스크립트나 일부 Docker Compose 버전에서 지원될 수 있지만, Python의 `os.environ.get`은 직접적으로 이를 해석하지 않습니다. `SWAP_LIMIT` 값을 명시적으로 설정하는 것이 (예: `SWAP_LIMIT=64G`) 가장 확실합니다.
    ...
    services:
      swap-manager:
        ...
        volumes:
          ...
          - /mnt/SwapWork:/mnt/SwapWork:rwx
          - /mnt/swap-manager:/var/log/my_app:rwx # 로그 및 .env 파일 경로
          - /etc/localtime:/etc/localtime:ro
        env_file:
          - /mnt/swap-manager/swap.env
        environment:
...
### 방법 2: 미리 빌드된 Docker Hub 이미지 사용 (`docker.io/nerdnam/swap-manager:latest`)
...
2.  **Docker Compose를 사용하여 실행 (권장):**
    `image` 지시어를 사용합니다. `env_file` 및 `volumes` 경로는 "공통 준비 사항"에서 생성한 실제 경로로 지정합니다.

    ```yaml
    # docker-compose.yaml (미리 빌드된 이미지 사용 예시)
    services:
      swap-manager:
        image: docker.io/nerdnam/swap-manager:latest
        container_name: swap-manager
        privileged: true
        pid: host
        ports:
...
3.  **`docker run` 명령어를 사용하여 직접 실행:**
    ...
    docker run -d \
      ...
      --restart unless-stopped \
      docker.io/nerdnam/swap-manager:latest
    ```
...
2.  **로그 확인:**
    ...
    ```
    호스트에 마운트된 로그 파일 확인 (위 예시에서는 `/mnt/swap-manager/swap_manager.log`):
    ```bash
